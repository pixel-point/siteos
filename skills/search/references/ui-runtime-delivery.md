...
```bash
node .agents/skills/search/scripts/ensure-typescript-config.mjs --project-root "$PWD"
```
...
- the SiteOS API base URL is server-side only and defaults to `https://siteos.xui.se`

For production client projects, the SiteOS API base URL must remain the production URL. Local or staging validation may adapt the committed server route artifact to a non-production base URL from `.siteos/project.json.apiBaseUrl`, but that value must stay server-side and must not create a public env contract.

Use the environment token helper for server-side query smoke verification. Do not read `.env` manually and do not write inline Node HTTP clients for the SiteOS environment query:

```bash
node .agents/skills/search/scripts/runtime-token-tooling.mjs validate \
  --project-root "$PWD" \
  --environment prod

node .agents/skills/search/scripts/runtime-token-tooling.mjs init \
  --project-root "$PWD" \
  --environment prod \
  --token-name "SiteOS Search UI"

node .agents/skills/search/scripts/runtime-token-tooling.mjs query \
  --project-root "$PWD" \
  --environment prod \
...
The `query` command reads `SITEOS_SEARCH_TOKEN` from the selected server env file and prints a redacted JSON result. Treat `status: "query-ok"` as enough evidence for server-side runtime query reachability.
...
- `NEXT_PUBLIC_SITEOS_API_BASE_URL`
