...
const buttonVariants = cva(
  cn(
    "relative inline-flex w-fit items-center justify-center gap-1 whitespace-nowrap text-sm font-medium tracking-tight transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 [&_a]:relative [&_a]:z-10",
  ...
  ),
  {
    variants: {
      ...
      size: {
        default:
          "h-10 b-rounded-md px-4 text-sm leading-none lg:h-11 lg:text-base lg:leading-none",
...
function Button({
  ...
  if (asChild && React.isValidElement(children)) {
    ...
    return React.cloneElement(elementToClone, {
      ...
      children: (
        <span className={baseTextClasses}>{elementToClone.props.children}</span>
      ),
    });
  }
