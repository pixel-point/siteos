...
Deliver `src/lib/siteos-search-analytics.ts` alongside the query client and dialog. Preserve the host’s visual design and adapt existing primitives. The helper is optional for collection but required by the delivered dialog import; collection follows the explicitly checked index mode and backend enablement; independent mode requires no banner. See [search-experience.md](search-experience.md) for the consent adapter and installation verification commands.
...
Verify late responses, empty/error states, pagination, mouse and Enter clicks, Escape focus return and reduced motion. Check independent collection without a banner, and no events with opt-out/GPC/DNT. In consent-required mode check no analytics request or interaction identifier before consent or after revocation, and that query/click receipts are never printed. Finish with `siteos search installation verify` through the actual local or hosted website endpoint; direct runtime reachability alone does not prove the proxy.
