...
- `schemaVersion: 1`
- `project.apiBaseUrlEnv: "SITEOS_SEARCH_PUBLIC_URL"`
...
- submit exactly one document payload to the selected environment without diagnostics polling or fallback
- classify submission outcomes with stable codes such as `api-unreachable`, `indexing-credential-rejected`, `selected-environment-unavailable`, `sync-submit-failed`, and `sync-submitted`; an explicit-environment `404` must not claim that the environment is absent or trigger a fallback
- never send `Authorization`, read `SITEOS_PROJECT_API_KEY`, call a credential-management endpoint, or print credentials and raw project configuration
...
Before the first live push, rerun `npx @siteoshq/cli search diagnostics --environment <slug> --json`. Inspect indexing credential metadata with `npx @siteoshq/cli search indexing-credential list --environment <slug> --json`; issue and install one with the matching `issue --install --json` command when none is suitable, or use `rotate --install --json` only for intentional replacement. Run diagnostics again after the submitted push to evaluate readiness. The sync runner does not issue credentials, call management endpoints, poll diagnostics, or use a Project API key.

When UI delivery needs a query credential, inspect metadata with `npx @siteoshq/cli search credential list --environment <slug> --json`; use the matching `issue --install --json` or intentional `rotate --install --json` command. The internal `scripts/runtime-token-tooling.mjs` helper only validates and queries with the installed `SITEOS_SEARCH_TOKEN` and exact `SITEOS_SEARCH_ENV`. Do not create browser-public search credential, environment-slug, or SiteOS API-base variables in any framework; `NEXT_PUBLIC_*` is prohibited for these values in Next.js projects.
...
- that unified CLI commands own query and indexing credential acquisition
- that `pnpm search:sync` submits the selected environment after source extraction and an explicit diagnostics check
- that `pnpm search:sync --dry-run` is available for explicit local document preview/debugging
- the next technical step: source extraction and explicit local document preview, then diagnostics, canonical indexing-credential installation, the first live sync, and a diagnostics rerun for the selected environment
