...
Before publication, show a concise field/region/service diff against `publishedConfig` from status, outstanding warnings and the affected website/environment. Cosmetic changes preserve consent choices; material policy changes may require visitors to choose again. With runtime 11.7+, unexpired purpose refusals and manual privacy restrictions survive a policy/region change without extending expiry; old positive grants are not copied to the new policy. Verify the deployed runtime before relying on this behavior. Do not equate a publishable configuration with a verified installation.
...
For custom code or non-Google GTM tags, read [Service permissions and transitions](references/service-permissions.md).
Use the exact service permission, keep initialization separate from conversions, and verify withdrawal.
Google `adsGranted` is not a general marketing permission. The browser API is not a hosted MCP/CLI command.

For native GA4 withdrawal control in Cookie 11.5+, set `draft.integrations.ga4MeasurementIds` to the verified IDs (maximum 20) in the full draft, select `google-analytics`, and publish when authorized. Keep first-execution consent triggers; this setting does not install GA4. Do not combine competing native and Custom HTML flag owners.
...
Check `SiteOSCookie.getRuntimeStatus()` and `siteos-cookie:lifecycle` as well as the GTM result. The loader retries script failures; the runtime retries transient configuration failures with bounded deadlines. A terminal error exposes Retry privacy controls. Test manual recovery. GTM bootstrap success is not Cookie readiness; a saved choice or essential-only policy can correctly produce readiness without showing a banner.
...
The CLI does not switch a live visitor's policy/region or test repeated-grant deduplication. Verify those
transitions separately using controlled fixtures or an authorized publication; a CLI pass alone does not cover them.
