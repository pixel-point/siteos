...
- Raw query text, raw query events, client identifiers, credentials, bearer headers, environment tokens, Meilisearch keys, raw `.siteos/search/project.json`, and private bindings are excluded from both chat answers and report artifacts.

## Analytics Fields To Use

Use these analytics response sections:
...
Use these diagnostics response sections when needed:
...
- `analytics-project-required`: Search Project selection is unavailable; delegate repair to `$siteos-search`.
- `analytics-api-unreachable`: the API base URL cannot be reached.
- `analytics-auth-rejected`: central Auth or Search grant authorization is rejected.
...
- Auth sessions or service grants
- authorization headers or tokens
- environment query credentials
- Meilisearch keys
- raw `.siteos/search/project.json` or private bindings
- raw query events, query text, or client identifiers

It is safe to report Search Project readiness, the safe Project name/slug, the selected environment slug, and redacted CLI command status.
