...
- `analytics`: safe JSON from `npx @siteoshq/cli search analytics --environment <environmentSlug> --json`.
- `generatedAt`: ISO timestamp. If omitted, the renderer may generate one.

Optional fields:

- `diagnostics`: safe JSON from `npx @siteoshq/cli search diagnostics --environment <environmentSlug> --json`.
...
Do not include credentials, bearer headers, raw `.siteos/search/project.json`, environment query credentials, Meilisearch keys, raw query events, raw query text, or client identifiers.
...
- fail with a clear safety error if `apiKey`, `Authorization`, `Bearer`, `siteos_`, `meili`, raw query event arrays, raw query text markers, raw `.siteos/search/project.json`, `http://`, `https://`, `<script src=`, `<link href=`, CDN references, or `Chart.js` survive into the generated HTML
...
- scan generated HTML for obvious secret markers such as `apiKey`, `Authorization`, `Bearer`, `siteos_`, `meili`, raw query event arrays, raw query text markers, or raw `.siteos/search/project.json`
